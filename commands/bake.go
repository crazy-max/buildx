...
func runBake(dockerCli command.Cli, targets []string, in bakeOptions) (err error) {
	...
	if url != "" {
		files, inp, err = bake.ReadRemoteFiles(ctx, drivers, url, in.files, printer)
	} else {
	...
	resp, err := build.Build(ctx, drivers, bo, dockerutil.NewClient(dockerCli), confutil.ConfigDir(dockerCli), printer)
	if err != nil {
