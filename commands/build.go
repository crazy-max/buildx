...
func runBuild(dockerCli command.Cli, in buildOptions) (err error) {
	...
	imageID, res, err := buildTargets(ctx, dockerCli, drivers, map[string]build.Options{defaultTargetName: opts}, in.progress, in.metadataFile, in.invoke != "")
	...
	if in.invoke != "" {
		...
		err = monitor.RunMonitor(ctx, cfg, func(ctx context.Context) (*build.ResultContext, error) {
			_, rr, err := buildTargets(ctx, dockerCli, drivers, map[string]build.Options{defaultTargetName: opts}, in.progress, in.metadataFile, true)
			return rr, err
...
func buildTargets(ctx context.Context, dockerCli command.Cli, drivers []builder.Driver, opts map[string]build.Options, progressMode string, metadataFile string, allowNoOutput bool) (imageID string, res *build.ResultContext, err error) {
	...
	var mu sync.Mutex
	var idx int
	resp, err := build.BuildWithResultHandler(ctx, drivers, opts, dockerutil.NewClient(dockerCli), confutil.ConfigDir(dockerCli), printer, func(driverIndex int, gotRes *build.ResultContext) {
		mu.Lock()
		defer mu.Unlock()
