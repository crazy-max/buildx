...
func runLs(dockerCli command.Cli, in lsOptions) error {
	...
	if printErr {
		...
		for _, b := range builders {
			if b.err != nil {
				_, _ = fmt.Fprintf(dockerCli.Out(), "Cannot load builder %s: %s\n", b.ng.Name, b.err.Error())
			} else {
				for idx, n := range b.ng.Nodes {
					...
					if nodeErr != "" {
						_, _ = fmt.Fprintf(dockerCli.Out(), "Failed to get status for %s (%s): %s\n", b.ng.Name, n.Name, nodeErr)
					}
				}
			}
		}
