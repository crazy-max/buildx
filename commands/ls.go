...
func runLs(dockerCli command.Cli, in lsOptions) error {
	...
	for _, b := range builders {
		func(b *builder.Builder) {
			eg.Go(func() error {
				_, _ = b.LoadDrivers(timeoutCtx, true)
				return nil
			})
		}(b)
	}
...
func printBuilder(w io.Writer, b *builder.Builder) (ok bool) {
	...
	if b.Err() == nil {
		for _, d := range b.Drivers() {
			...
			if d.Err != nil {
				ok = false
				fmt.Fprintf(w, "  %s\t%s\t%s\t\t\n", d.Name, d.Endpoint, "error")
			} else {
				fmt.Fprintf(w, "  %s\t%s\t%s\t%s\t%s\n", d.Name, d.Endpoint, status, d.Version, strings.Join(platformutil.FormatInGroups(d.Node.Platforms, d.Platforms), ", "))
			}
		}
	}
	return
