...
func (ls *LocalState) Txn() (*Txn, func(), error) {
	l := flock.New(filepath.Join(ls.root, refsDir, ".lock"))
...
func (t *Txn) RemoveRef(builderName, nodeName, id string) error {
	if err := t.ls.validate(builderName, nodeName, id); err != nil {
		return err
	}
...
func (t *Txn) RemoveBuilderRefs(builderName string) error {
	if builderName == "" {
...
func (t *Txn) RemoveNodeRefs(builderName string, nodeName string) error {
	if builderName == "" {
		return fmt.Errorf("builder name empty")
	}
	if nodeName == "" {
